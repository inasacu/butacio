contract Butacio{
    ...
    address _owner;
    uint public _numEvents;
    Event[] _events;

    // Tokens to exchange to EUR
    ButacioToken public _tokens = new ButacioToken();
    ...
    function getEvent(uint eventId) constant return(uint, uint){

      return (_events[eventId].numTickets, _events[eventId].endDate);
    }

    function getTicket(uint eventId, uint ticketId) constant return(address, bool, uint, bool){

      return (
        _events[eventId].tickets[ticketId].owner,
        _events[eventId].tickets[ticketId].used,
        _events[eventId].tickets[ticketId].price,
        _events[eventId].tickets[ticketId].onSale
      );
    }
}
